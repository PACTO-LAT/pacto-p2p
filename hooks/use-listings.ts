...
export function useUserListings(userId?: string) {
  return useQuery({
    ...
    queryFn: () => (userId ? ListingsService.getUserListings(userId) : Promise.resolve([])),
    enabled: !!userId,
  })
}
...
export function useCreateListing() {
  ...
  return useMutation({
    mutationFn: ({ userId, listingData }: { userId: string; listingData: CreateListingData }) =>
...
export function useUpdateListing() {
  ...
  return useMutation({
    mutationFn: ({ listingId, updates }: { listingId: string; updates: Partial<Listing> }) =>
