...
export class TrustlessWorkService {
  ...
  static async releaseTokens(contractId: string): Promise<string> {
    // Mock token release
    await new Promise((resolve) => setTimeout(resolve, 2000))
    return `release_${Date.now()}`
  }

  static async createDispute(contractId: string, reason: string): Promise<void> {
    // Mock dispute creation
    await new Promise((resolve) => setTimeout(resolve, 1000))
    console.log(`Dispute created for contract: ${contractId}, reason: ${reason}`)
  }

  static async getContractStatus(contractId: string): Promise<TrustlessWorkContract["status"]> {
