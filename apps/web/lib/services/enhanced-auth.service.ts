...
export namespace EnhancedAuthService {
  ...
  export async function updateUserProfile(
    ...
    if (!validation.success) {
      const errorMessages = validation.error.issues
        .map(
          (err) =>
            `${err.path.filter((p) => typeof p !== 'symbol').join('.')}: ${err.message}`
        )
        .join('; ');
    ...
    try {
      ...
      if (
        validatedData.email &&
        !validatedData.email.endsWith('@wallet.local')
      ) {
    ...
    try {
      ...
      if (!data) {
        throw new ProfileUpdateError('User not found', 'USER_NOT_FOUND', {
          userId,
        });
      }
  ...
  export function validateField(
    ...
    const fieldError = schema.error.issues.find((err) => err.path[0] === field);
