...
export function getEscrowRole(escrow: Escrow, userAddress: string): 'buyer' | 'seller' | null {
...
export function canReportPayment(escrow: Escrow, userRole: 'buyer' | 'seller'): boolean {
...
export function canConfirmPayment(escrow: Escrow, userRole: 'buyer' | 'seller'): boolean {
  if (userRole !== 'seller') return false;
  return !escrow.milestones[0].approved;
}

export function canDeposit(escrow: Escrow, userRole: 'buyer' | 'seller'): boolean {
  if (userRole !== 'seller') return false;
  if (escrow.flags?.released || escrow.flags?.resolved) return false;
  return escrow.balance === 0;
}

export function canReleaseFunds(escrow: Escrow, userRole: 'buyer' | 'seller'): boolean {
  if (userRole !== 'seller') return false;
...
export function canDispute(escrow: Escrow): boolean {
  if (escrow.flags?.disputed || escrow.flags?.resolved || escrow.flags?.released) return false;
  return escrow.balance !== 0;
}
